...
(defun test-bracket-arg ()
  (let* ((str "ADD([[xx yy]])")
	...
	(setf tmp (nth 0 prod))
	(assert (= (length tmp) 2))
	(assert (string= (nth 0 tmp) "ADD"))
	(assert (string= (nth 1 tmp) "xx yy"))))
...
(defun test-arg-combination ()
  (let* ((str "ADD(xx=\"yy zz\"[[aabb]])")
		 (prod (parse-string str))
		 tmp)
	(assert (= (length prod) 1))
	(setf tmp (nth 0 prod))
	(assert (= (length tmp) 2))
	(assert (string= (nth 0 tmp) "ADD"))
	(assert (string= (nth 1 tmp) "xx=\"yy zz\"[[aabb]]"))))

(defun test-arg-combination2 ()
  (let* ((str "ADD(xx=[[aa bb]])")
		 (prod (parse-string str))
		 tmp)
	(assert (= (length prod) 1))
	(setf tmp (nth 0 prod))
	(assert (= (length tmp) 3))
	(assert (string= (nth 1 tmp) "xx=[[aa"))
	(assert (string= (nth 2 tmp) "bb]]"))))
...
(defun test-all ()
  ...
  (test-bracket-arg)
  (test-quoted-arg)
  (test-arg-combination)
  (test-arg-combination2)
  (test-quoted-encoded)
